        ...
        ws.onPriceUpdate((data) => {
          ...
          setWatchlist(prev => prev.map(stock => {
            ...
            if (Math.abs(newPrice - oldPrice) >= 0.01) {
              const direction = newPrice > oldPrice ? 'up' : 'down'; // FIXED: green for up, red for down
    ...
    try {
      ...
      updatedStocks.forEach((newStock, index) => {
        ...
        if (oldStock && Math.abs(newStock.price - oldStock.price) >= 0.01) {
          const direction = newStock.price > oldStock.price ? 'up' : 'down'; // FIXED: green for up, red for down
